    ...
    <div className="max-w-site mx-auto px-4 sm:px-6 py-16">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
        ...
        <h1 className="heading-md mb-3">
          <span className="text-[var(--color-text)]">{t("faq.title_1")}</span>
          <span className="text-gradient">{t("faq.title_2")}</span>
        </h1>
        <div className="max-w-xs mx-auto mt-4">
          <div className="relative">
            <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]" />
            <input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder={t("faq.search")} className="input pl-8" />
          </div>
        </div>
      </motion.div>
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
            <button onClick={() => setOpen(open === i ? null : i)}
              className={`w-full text-left glass-card p-4 transition-all duration-300 ${
                open === i ? "border-[var(--color-primary)]/20" : ""
              }`}
            >
              <div className="flex items-center justify-between gap-3">
                <span className="text-[var(--color-text)] text-sm font-medium">{faq.q}</span>
                <ChevronDown size={14}
                  className={`text-[var(--color-text-muted)] shrink-0 transition-transform duration-300 ${open === i ? "rotate-180" : ""}`} />
              </div>
              <AnimatePresence>
                {open === i && (
                  <motion.p initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                    exit={{ height: 0, opacity: 0 }}
                    className="text-[var(--color-text-muted)] text-xs font-mono mt-3 leading-relaxed overflow-hidden"
                  >
                    {faq.a}
                  </motion.p>
                )}
