...
app.get("/api/reviews", async (req, res) => {
  try {
    ...
    let reviews = [];
    if (productId) {
      const { rows } = await query("SELECT * FROM reviews WHERE \"productId\" = $1 ORDER BY \"createdAt\" DESC", [productId]);
      reviews = rows;
    } else {
      const { rows } = await query("SELECT * FROM reviews ORDER BY \"createdAt\" DESC");
      reviews = rows;
    }
...
if (existsSync(DIST_DIR)) {
  ...
  console.log("Dev mode: no dist folder, API-only mode (Vite proxy active)");
}
    ...
    if (!rows.length) {
      ...
      await query(
        `INSERT INTO users (id, name, email, password, role, avatar, addresses, wishlist, "createdAt")
         VALUES ($1,$2,$3,$4,$5,$6,'[]'::jsonb,'[]'::jsonb,$7)`,
        ["admin-root", "Admin RT", "admin@rtelectronics.com", hashed, "admin",
         "https://api.dicebear.com/7.x/avataaars/svg?seed=admin", new Date().toISOString()]
      );
      console.log("Default admin auto-created (admin@rtelectronics.com / admin123)");
    }
  } catch (e) { console.log("Admin check skipped:", e.message); }
